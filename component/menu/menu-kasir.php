        ...
        <?php while($row = mysqli_fetch_assoc($show_menu)) : ?>
            <div class="flex flex-col py-2 px-4 border rounded-lg bg-gray-700">
                ...
                <div class="flex justify-end h-full items-end flex-row">
                    <a href="order-menu.php?id=<?= $row["id_menu"] ?>" class="w-full text-white text-center py-2 px-4 bg-sky-600 hover:bg-sky-700 duration-300 font-bold text-lg mr-3 rounded">Order</a>
                </div>
            </div>
        <?php endwhile?>
    </div>
